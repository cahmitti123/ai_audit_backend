...
echo ""
echo "1. Copy example: cp .env.example ${ENV_FILE}"
...
ENV_FILE="${ENV_FILE}" ${COMPOSE} -f "${COMPOSE_FILE}" --env-file "${ENV_FILE}" up -d --build --remove-orphans --scale server="${SERVER_REPLICAS}"

echo ""
echo "✅ Deployment complete."
echo ""
echo "Logs:"
echo "  ENV_FILE=${ENV_FILE} ${COMPOSE} -f ${COMPOSE_FILE} --env-file ${ENV_FILE} logs -f --tail=100"
echo ""
