...
async function main() {
  ...
  // ============================================================================
  // 0. AUTH (RBAC) — roles, permissions, optional admin user
  // ============================================================================
  console.log("\n🔐 Seeding auth roles/permissions...");

  // IMPORTANT:
  // Permission keys are stored as BASE keys (no ".read"/".write" suffix).
  // Route guards accept "audits.read"/"audits.run"/etc and map to base key + read/write grant.
  ...
  if (adminEmail && adminPassword) {
    ...
    const user = existing
      ? await prisma.user.findUniqueOrThrow({ where: { id: existing.id }, select: { id: true, email: true } })
